...
static_assert(sizeof(FixedSizePos) == 92, "");
struct FixedSizePosAndEval {
	FixedSizePos fixedSizePos;
	s16 eval;
	s16 padding;
};
...
public:
	...
	void set(const FixedSizePos& fsp, Thread* th);
	...
	FixedSizePos toFixedSizePos(const Ply ply) const;
	FixedSizePos toFixedSizePos() const { return toFixedSizePos(gamePly()); }
