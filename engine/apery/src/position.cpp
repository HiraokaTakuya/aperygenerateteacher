...
FixedSizePos Position::toFixedSizePos(const Ply ply) const {
	FixedSizePos result;
	result.hand[Black] = hand(Black);
	result.hand[White] = hand(White);
	result.ply = ply;
	for (Square sq = SQ11; sq < SquareNum; ++sq)
		result.board[sq] = piece(sq);
	result.turn = turn();
	return result;
}
...
void Position::set(const FixedSizePos& fsp, Thread* th) {
	Searcher* s = std::move(searcher_);
	clear();
	setSearcher(s);

	for (Square sq = SQ11; sq < SquareNum; ++sq)
		if (static_cast<Piece>(fsp.board[sq]) != Empty)
			setPiece(static_cast<Piece>(fsp.board[sq]), sq);

	kingSquare_[Black] = bbOf(King, Black).constFirstOneFromSQ11();
	kingSquare_[White] = bbOf(King, White).constFirstOneFromSQ11();
	goldsBB_ = bbOf(Gold, ProPawn, ProLance, ProKnight, ProSilver);

	hand_[Black] = fsp.hand[Black];
	hand_[White] = fsp.hand[White];
	turn_ = static_cast<Color>(fsp.turn);
	gamePly_ = fsp.ply;
